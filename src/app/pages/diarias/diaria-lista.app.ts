...
@Component({
  ...
  template: `
    <div class="container">
      <div class="header">
        <h1>Diárias</h1>
        <button class="btn-primary" (click)="novo()">+ Nova Diária</button>
      </div>
      ...
      <div *ngIf="!loading && diariasFiltradas.length === 0" class="empty">
        Nenhuma diária encontrada
      </div>

      <div class="table-container" *ngIf="!loading && diariasFiltradas.length > 0">
        <table>
          <thead>
            <tr>
          ...
          <tbody>
            <tr *ngFor="let diaria of diariasFiltradas">
              <td><strong>Tipo {{ diaria.tipoApartamento?.tipo }}</strong></td>
              <td>{{ diaria.quantidade }}</td>
              <td>{{ diaria.valor | currency:'BRL' }}</td>
              <td>
                <button class="btn-edit" (click)="editar(diaria.id!)">Editar</button>
                <button class="btn-delete" (click)="excluir(diaria.id!)">Excluir</button>
              </td>
            </tr>
          </tbody>
        </table>
...
export class DiariaListaApp implements OnInit {
  ...
  filtrar(): void {
    ...
    this.diariasFiltradas = this.diarias.filter(d =>
      d.tipoApartamento?.tipo?.toLowerCase().includes(termo)
    );
  }
  ...
  excluir(id: number): void {
    if (confirm('Deseja realmente excluir esta diária?')) {
      this.diariaService.delete(id).subscribe({
        next: () => {
          this.carregarDiarias();
        },
        error: (err) => {
